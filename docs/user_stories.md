...
- **US3.3:** As a user, I want to choose a payment method (COD or Bank Transfer/QR) so that I can pay conveniently.
- **US3.4:** As a user, I want to confirm my order and have it synced to the cloud so that the restaurant receives it.
- **US3.5:** As a user, I want to view my order history by pulling down to refresh so that I can get the latest status from the cloud.
...
- **US5.3:** As an admin, I want to toggle the active status of products (isActive) so that I can temporarily hide items that are out of stock.
